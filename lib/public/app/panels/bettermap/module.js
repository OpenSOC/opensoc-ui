        ...
        results.then(function(results) {
          ...
          if($scope.query_id === query_id) {
            ...
            $scope.data = $scope.data.slice(0,$scope.panel.size).concat(_.map(results.hits.hits, function(hit) {
              ...
              // var parsedCoord = string_variable.replace(/"[^"]*"/g, '').split(',');
              var parsedCoord = hit.fields[$scope.panel.field][0].split(',');
              // console.log(parsedCoord);
              var lonCoord = parsedCoord[0].toString();
              var latCoord = parsedCoord[1].toString();
              console.log(lonCoord, latCoord);

              return {
                // coordinates : new L.LatLng(hit.fields[$scope.panel.field][1],hit.fields[$scope.panel.field][0]),
                coordinates : new L.LatLng(lonCoord,latCoord),
