  ...
  module.controller('pcap', function($scope, $http, $rootScope, $modal, $q, $compile, $timeout, $routeParams, fields, querySrv, filterSrv, dashboard) {
    ...
    $scope.search = function() {
      ...
      $scope.results = client
        .query(
          ejs.BoolQuery()
            .should($scope.ejs.TermQuery('ip_src_addr', $scope.ip_src_addr))
            .should($scope.ejs.TermQuery('ip_dst_addr', $scope.ip_dst_addr))
            .should($scope.ejs.TermQuery('ip_src_port', $scope.ip_src_port))
            .should($scope.ejs.TermQuery('dst_port', $scope.dst_port))
            .should($scope.ejs.TermQuery('ip_protocol', $scope.ip_protocol))
        )
        .doSearch();
