...
exports.initialize = function (app, server) {
  ...
  wss.on('connection', function (ws) {
    ...
    redisClient.hkeys(countKey, function (err, keys) {
      _.each(keys, function (key) {
        var count, value, timestamp;

        // callback hell :-(
        // build alert then send as message to websockets client
        redisClient.hget(countKey, key, function (err, result) {
          count = result;
          redisClient.hget(valueKey, key, function (err, result) {
            value = result;
            redisClient.hget(timestampKey, key, function (err, result) {
              var alert = {
                count: count,
                value: value,
                timestamp: result
              };
              ws.send(JSON.stringify(alert), function () {} );
            });
          });
        });
      });
